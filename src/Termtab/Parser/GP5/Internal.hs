...
getMeasureHeader idx (prevNum, prevDen) (prevRoot, prevMode) = do
  flags <- getU8
  (timeNum, timeDen) <- if flags `testBit` 0
    then do
      n <- getU8
      d <- getU8
      return (n, d)
  ...
  (keyRoot, keyMode) <- if flags `testBit` 4
    then do
      r <- getInt8'
      m <- getU8
      return (r, m)
...
getTrack programs = do
  ...
  _height  <- getInt32le'
  skip 4   -- color
  let prog = if chanIdx >= 1 && chanIdx <= length programs
...
getBeat = do
  flags  <- getU8
  isRest <- if flags `testBit` 6  -- 0x40
              then (\s -> s == 0x02) <$> getU8
              else return False
  durRaw <- getInt8'
